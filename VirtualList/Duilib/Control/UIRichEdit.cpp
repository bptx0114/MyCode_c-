...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
	...
	m_dwTipValueColor(0xFFBAC0C5), m_uTipValueAlign(DT_SINGLELINE | DT_LEFT), m_bAscii(false)
{
#ifndef _UNICODE
...
void CRichEditUI::SetAscii(bool bAscii/* = false*/)
{
	m_bAscii = bAscii;
}

bool CRichEditUI::IsAscii()
{
	return m_bAscii;
}
...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcsicmp(pstrName, _T("IsAscii")) == 0) SetAscii(_tcsicmp(pstrValue, _T("true")) == 0);
