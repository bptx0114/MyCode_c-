...
namespace DuiLib {
	...
	bool CListUI::SelectRange(int iIndex, bool bTakeFocus /* = false */)
	{
		...
		UnSelectAllItems();//去掉所有选中
		for (int i = 0; i < GetCount()/*m_items.GetSize()*/; i++)
		{
			if (m_items[i] != NULL)
			{
				if (i >= m_iSelBegin && i <= m_iSelEnd)//在范围内的选中
				{
					SelectMultiItem(i, true);
				}
			}
		}
		m_iCurSel = iOldSel;
