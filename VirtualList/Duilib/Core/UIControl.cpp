...
namespace DuiLib {
	...
	CDuiString CControlUI::GetHotText() const
	{
		if (!IsResourceText()) return m_sHotText.IsEmpty() ? GetText() : m_sHotText;
		return CResourceManager::GetInstance()->GetText(m_sHotText);
	}

	void CControlUI::SetHotText(LPCTSTR pstrText)
	{
		if (m_sHotText == pstrText) return;
	...
	void CControlUI::SetBorderSize(int nSize)
	{
		...
		m_nBorderSize = nSize;
		m_rcBorderSize.left = m_rcBorderSize.top = m_rcBorderSize.right = m_rcBorderSize.bottom = nSize;
		Invalidate();
	}
...
// 				SetBorderSize(rcPadding);
		...
		else if (_tcsicmp(pstrName, _T("hottext")) == 0) SetHotText(pstrValue);
